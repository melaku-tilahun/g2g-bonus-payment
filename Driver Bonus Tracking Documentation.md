...
- **Two-Stage Reconciliation**: Batch export for Telebirr Bulk Pay, followed by automated "Paid" status updates via report upload.
...
graph TD
    ...
    B --> E[Audit Logger]
    B --> F[public/uploads/driver_photos]
    B --> G[ExcelJS Parser]
```
...
- **Storage**: Photos stored on disk (`/uploads/driver_photos`) for performance; metadata in MySQL.
...
### Staff Role
- Excel Imports: Upload and validate weekly bonus files.
...
- **Photo Storage**: Driver photos saved to disk storage (`/uploads/driver_photos`) for performance.
...
- **Report Reconciliation**: Upload Telebirr success reports to bulk-update database statuses.
...
3. **Reconcile**: Upload the bank's "Success Report". System matches by MSISDN, sets status to `paid`, and updates `payment_date` to the current reconciliation timestamp.
...
CREATE TABLE drivers (
  ...
  manager_photo VARCHAR(255) NULL, -- Path to file (/uploads/driver_photos/...)
...
- **POST /api/payments/reconcile**: Upload Telebirr report to mark as `paid`.
- **GET /api/payments/history**: Searchable history of all past payouts.

### 5.2.4 Upload & Excel Management

- **POST /api/uploads/validate**: Dry-run validation of a monthly bonus file.
- **POST /api/uploads/excel**: Final import of bonus rows.
- **GET /api/uploads/history**: Audit log of all file uploads.
...
4. Upload the bank's **Success Report**.
...
- [ ] **Upload Permissions**: Ensure `public/uploads/driver_photos` is writable.
