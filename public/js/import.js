document.addEventListener("DOMContentLoaded", () => {
  ...
  importForm.onsubmit = async (e) => {
    ...
    try {
      ...
      const result = await api.post("/imports/excel", formData);
      ...
      ui.toast(error.message || "Import failed", "error");
      importBtn.disabled = false;
      importBtn.innerHTML =
        '<i class="fas fa-file-import me-2"></i> Start Import Process';
    }
...
async function loadHistory() {
  try {
    ...
    if (!history.length) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="text-center py-4 text-muted">No import history found</td></tr>';
      return;
    }
    ...
    history.forEach((h) => {
      ...
      tr.innerHTML = `
                ...
                <td class="fw-semibold">
                    ${
                      h.file_path
                        ? `<a href="/imports/${h.file_path}" target="_blank" class="text-decoration-none"><i class="fas fa-download me-1"></i> ${h.file_name}</a>`
                        : h.file_name
                    }
                </td>
